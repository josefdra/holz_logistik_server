			...
			Ok(msg) => {
				if let Some(text) = msg.to_str().ok() {
					if let Ok(json_msg) = serde_json::from_str::<Value>(text) {
						if json_msg.get("type").and_then(|v| v.as_str()) == Some("authentication_request") {
							let data = json_msg.get("data").cloned().unwrap_or(json!({}));
							return handle_authentication_request(client_id, clients, db_pools, data).await;
						}
					}
				}
			}
							...
							if handle_sync_request(&data, client_id.clone(), &clients).await {
								...
								let should_send_message = {
									match clients.lock() {
										...
										Err(e) => {
											println!("Failed to lock clients to update sync status: {:?}", e);
											false
										}
									}
								};
...
async fn main() -> Result<()> {
	...
	if !dir_path.exists() {
		eprintln!("Databases directory missing");
	}
