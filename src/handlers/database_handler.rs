...
use crate::local_storage::{
    core_local_storage::CoreLocalStorage,
    contract::contract_local_storage::ContractLocalStorage,
    location::location_local_storage::LocationLocalStorage,
    note::note_local_storage::NoteLocalStorage,
    photo::photo_local_storage::PhotoLocalStorage,
    sawmill::sawmill_local_storage::SawmillLocalStorage,
    shipment::shipment_local_storage::ShipmentLocalStorage,
    user::user_local_storage::UserLocalStorage,
};
...
impl DatabaseHandler {
    ...
    pub async fn get_or_create_pool(&self, tenant: &str) -> Result<DbPool, DatabaseError> {
        ...
        log::info!("Creating new connection pool for tenant {} at {}", tenant, db_path);
    ...
    fn handle_contract_update(
        ...
        if !is_deleted {
            let contract_storage = ContractLocalStorage::new(core_storage)
                .map_err(|e| DatabaseError::Storage(e.to_string()))?;
            contract_storage.save_contract(data)
                .map_err(|e| DatabaseError::Storage(e.to_string()))
        } else {
    ...
    fn handle_location_update(
        ...
        if !is_deleted {
            let location_storage = LocationLocalStorage::new(core_storage)
                .map_err(|e| DatabaseError::Storage(e.to_string()))?;
            location_storage.save_location(data)
                .map_err(|e| DatabaseError::Storage(e.to_string()))
        } else {
    ...
    fn handle_note_update(
        ...
        if !is_deleted {
            let note_storage = NoteLocalStorage::new(core_storage)
                .map_err(|e| DatabaseError::Storage(e.to_string()))?;
            note_storage.save_note(data)
                .map_err(|e| DatabaseError::Storage(e.to_string()))
        } else {
    ...
    fn handle_sawmill_update(
        ...
        if !is_deleted {
            let sawmill_storage = SawmillLocalStorage::new(core_storage)
                .map_err(|e| DatabaseError::Storage(e.to_string()))?;
            sawmill_storage.save_sawmill(data)
                .map_err(|e| DatabaseError::Storage(e.to_string()))
        } else {
    ...
    fn handle_shipment_update(
        ...
        if !is_deleted {
            let shipment_storage = ShipmentLocalStorage::new(core_storage)
                .map_err(|e| DatabaseError::Storage(e.to_string()))?;
            shipment_storage.save_shipment(data)
                .map_err(|e| DatabaseError::Storage(e.to_string()))
        } else {
    ...
    fn handle_deletion(
        ...
        if let Some(id) = data.get("id").and_then(|v| v.as_str()) {
            core_storage.mark_as_deleted(table, id)
                .map(|_| true)
                .map_err(|e| DatabaseError::Storage(e.to_string()))
        } else {
