...
impl ConnectionHandler {
	...
	pub async fn handle_new_connection<C>(&self, ws: WebSocket, controller: Arc<C>)
	where
		C: ProcessMessage + Send + Sync + 'static,
	{
		...
		match auth_result {
			Ok(Ok(true)) => {
				...
				self
					.handle_authenticated_connection(client_id.clone(), ws_rx, controller)
					.await;
			}
			Ok(Ok(false)) => {
						...
						if let Ok(json_msg) = serde_json::from_str::<Value>(text) {
							...
							if msg_type == Some("authentication_request") {
								if let Err(e) = controller
									.process_message(client_id.clone(), json_msg)
									.await
								{
