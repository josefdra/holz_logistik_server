...
impl SyncService {
    ...
    pub async fn handle_sync_request(
        ...
        let client = self.client_handler
        ...
        let sync_dates = SyncDates {
            user: data.get("user_update").and_then(|v| v.as_i64()).unwrap_or(0),
            sawmill: data.get("sawmill_update").and_then(|v| v.as_i64()).unwrap_or(0),
            contract: data.get("contract_update").and_then(|v| v.as_i64()).unwrap_or(0),
            note: data.get("note_update").and_then(|v| v.as_i64()).unwrap_or(0),
            location: data.get("location_update").and_then(|v| v.as_i64()).unwrap_or(0),
            shipment: data.get("shipment_update").and_then(|v| v.as_i64()).unwrap_or(0),
            photo: data.get("photo_update").and_then(|v| v.as_i64()).unwrap_or(0),
        };

        // Send data for each entity type
        self.send_user_data(sync_dates.user, &client_id, core_storage.clone()).await?;
        self.send_sawmill_data(sync_dates.sawmill, &client_id, core_storage.clone()).await?;
        self.send_contract_data(sync_dates.contract, &client_id, core_storage.clone()).await?;
        self.send_location_data(sync_dates.location, &client_id, core_storage.clone()).await?;
        self.send_shipment_data(sync_dates.shipment, &client_id, core_storage.clone()).await?;
        self.send_note_data(sync_dates.note, &client_id, core_storage.clone()).await?;
        self.send_photo_data(sync_dates.photo, &client_id, core_storage.clone()).await?;

        // Mark sync as completed
        self.client_handler.mark_sync_completed(&client_id).await.ok();
    ...
    async fn send_photo_data(
        ...
        let photo_storage = PhotoLocalStorage::new(core_storage)
            .map_err(|e| SyncError::StorageError(e.to_string()))?;
