...
impl AuthService {
	pub fn new(database_handler: Arc<DatabaseHandler>, client_handler: Arc<ClientHandler>) -> Self {
		Self {
			database_handler,
			client_handler,
		}
	...
	pub async fn authenticate(
		...
		if !self.database_handler.database_exists(tenant).await {
			self
				.send_auth_rejection(&client_id, "Invalid tenant")
				.await?;
			return Ok(false);
		}
		...
		let core_storage = Arc::new(
			CoreLocalStorage::new(&db_path).map_err(|e| AuthError::StorageError(e.to_string()))?,
		);

		let user_storage =
			UserLocalStorage::new(core_storage).map_err(|e| AuthError::StorageError(e.to_string()))?;
		...
		if user_result.is_none() {
			self
				.send_auth_rejection(&client_id, "User not found")
				.await?;
			return Ok(false);
		}
	...
	async fn send_auth_rejection(&self, client_id: &str, error: &str) -> Result<(), AuthError> {
		let response = json!({
				"type": "authentication_response",
				"data": {
						"authenticated": 0,
...
impl From<crate::services::message_service::MessageError> for AuthError {
	fn from(err: crate::services::message_service::MessageError) -> Self {
		AuthError::MessageError(err.to_string())
	}
}
