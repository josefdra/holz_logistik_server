...
impl CoreLocalStorage {
    pub fn new(db_path: &str) -> Result<Self> {
        ...
        Ok(CoreLocalStorage { connection: conn })
    }
    ...
    pub fn get_by_column(&self, table_name: &str, column_name: &str, value: &str) -> Result<Vec<serde_json::Value>> {
    ...
    pub fn delete(&self, table_name: &str, id: &str) -> Result<usize> {
        let query = format!("DELETE FROM {} WHERE id = ?", table_name);
        self.connection.execute(&query, params![id])
    }
    
    pub fn delete_by_column(&self, table_name: &str, column_name: &str, value: &str) -> Result<usize> {
