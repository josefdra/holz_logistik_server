...
impl ContractLocalStorage {
    ...
    pub fn get_active_contracts(&self) -> Result<Vec<Contract>> {
        let query = format!("SELECT * FROM {} WHERE done = 0", ContractTable::TABLE_NAME);

        let conn = self.core_storage.get_connection();
        let _stmt = conn.prepare(&query)?;
    ...
    pub fn get_finished_contracts_by_date(
        &self,
        start: DateTime<Utc>,
        end: DateTime<Utc>,
    ) -> Result<Vec<Contract>> {
    ...
    pub fn get_contract_by_id(&self, id: &str) -> Result<Contract> {
        let json_values = self.core_storage.get_by_id(ContractTable::TABLE_NAME, id)?;
    ...
    pub fn delete_contract(&self, id: &str, done: bool) -> Result<usize> {
        // Get the contract before deleting it (for event notifications)
        let _contract = self.get_contract_by_id(id)?;

        // Delete from database
        let result = self.core_storage.delete(ContractTable::TABLE_NAME, id)?;
